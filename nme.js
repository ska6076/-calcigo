...
async function downloadFormOnly() {
  ...
  try {
    ...
    // ── NEW: Hide the icon strip so it never appears in the downloaded PDF ──
    if (layoutSection) {
      layoutPrevDisplay = layoutSection.style.display;
    ...
    if (layoutSection) {
      layoutSection.style.display = layoutPrevDisplay || "";
    }
...
async function mergePdfWithForm() {
  ...
  try {
    ...
    // ── NEW: Hide the icon strip so it never appears in the merged PDF ──
    if (layoutSection) {
      layoutPrevDisplay = layoutSection.style.display;
      layoutSection.style.display = "none";
    }
